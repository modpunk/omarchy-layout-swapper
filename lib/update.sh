...
# ---- this plugin -------------------------------------------------------------
UPD_ID="fans.omarchy.layout-swapper"
UPD_NAME="Layout Swapper"
UPD_REPO="modpunk/omarchy-layout-swapper"            # GitHub owner/repo the plugin is published from
UPD_BRANCH="main"        # branch whose manifest.json is "the published version"
UPD_SLUG="omarchy-layout-swapper"            # cache lives in ~/.cache/<slug>/update-check.json
UPD_CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/omarchy-layout-swapper/config.json"        # JSON file; "update_check": false turns the check off
...
post_update() {
  # The autosave daemon reloads its own code on the next login (post-boot hook);
...
cmd_terminal() {
  ...
  if (( UPD_KEEP_LOADED )); then
    ...
    echo "The bar picks up the new files by itself (restart the shell if the chip does not refresh)."
  fi
  echo "Done."
  sleep 2
}
