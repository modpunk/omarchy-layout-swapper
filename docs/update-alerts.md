...
- fetches `https://raw.githubusercontent.com/<owner>/<repo>/<branch>/manifest.json`
...
- Commits in Omarchy.Fans repos are authored `modpunk
  <27315771+modpunk@users.noreply.github.com>`. Merge by fast-forward pushing the
  locally rebased branch (`git push origin <branch>:main`); GitHub's merge,
  squash and rebase buttons all stamp the account's primary email as author or
  committer.
...
- Test the banner without touching the installed plugin: point
  `OMARCHY_PLUGIN_UPDATE_RAW` at a `file://` folder to test the fetch offline,
  fake a newer version by writing the cache file with `"latest": "9.9.9"` and
  `"checked"` set to now, and test the mismatch case against an older artifact.
- A network fetch changes the marketplace security baseline. When a listed
  plugin gains this, open a "Plugin verification" issue and add a short
...
- Kind: bar-only widget (`BarWidget.qml`). The Python CLI `bin/omarchy-layout-swapper`
  exposes `update-check`, `update-dismiss`, `update-run`, each of which execs
  `lib/update.sh`; the widget also calls `/usr/bin/bash lib/update.sh` directly.
- Repo: `modpunk/omarchy-layout-swapper`, branch `main`.
- Version places: `manifest.json` and `CHANGELOG.md` (the CLI reads the version
  from `manifest.json`; there is no separate constant).
- Cache: `~/.cache/omarchy-layout-swapper/update-check.json`. Opt-out:
  `"update_check": false` in `~/.config/omarchy-layout-swapper/config.json`, or
  in the widget's `shell.json` entry.
- The widget checks on load and every six hours, shows a dot, and the next click
  opens the popup (Update…, Later, Switch layout).
- No build artifact and no copied helper, so no mismatch case beyond a stale
  `omarchy plugin update` (the widget's version vs. the helper's): `install.sh`
  only symlinks the CLI into `~/.local/bin`. `UPD_KEEP_LOADED=0`; the bar picks
  up the new files on the next shell start.
